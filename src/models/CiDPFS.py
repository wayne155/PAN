...
class CiDBlock(nn.Module):
    def __init__(self,enc_in, patch_emb, d_model, mask=True, hidden_size=512, ti=95):
        ...
        self.linear = nn.Sequential(
            nn.Linear(patch_emb + d_model + enc_in, hidden_size),
            nn.ReLU(),
    ...
    def forward(self, patch, prev, lst):
        # patch : B, N, patch_emb
        # prev : B, N, d_model
        ...
        out = self.norm(prev + self.linear(inp))
        return out
...
class CiDP(nn.Module):
    def __init__(self, seq_len,pred_len,enc_in, mask=True, hidden_size=128, flatten=True,dropout=0.0,d_model=256,d_ff=512, patch_len=24, stride=16, task_name="long_term_forecast"):
        ...
        # self.share_cd = share_cd
        # self.share_patch = share_patch
        ...
        self.value_embedding = nn.Linear(patch_len, d_model, bias=False)
        self.enc_emb = nn.Linear(enc_in, d_model, bias=False)
        self.dropout = nn.Dropout(dropout)
        ...
        self.mid = nn.ModuleList(
            [CiDBlock(enc_in=enc_in, patch_emb=patch_len, d_model=d_model, mask=mask, hidden_size=d_ff, ti=i) for i in self.t_index]
        )
        if self.flatten:
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # lst = raw_x[:, : , self.t_index] # B, N, pn 
        # lst = lst.unsqueeze(1).repeat(1, N, 1) # B, N, N, pn
        # if self.mask:
        #     mask = (1 - torch.eye(N).unsqueeze(0).unsqueeze(-1)).to(x_enc.device) # 1, N, N, 1
        #     lst = lst * mask # B, N, N, pn

        # mask pass and repeat
        lst = raw_x[:, : , self.t_index] # B, N, pn 
        lst = lst.unsqueeze(1).repeat(1, N, 1, 1) # B, N, N, pn
        if self.mask:
            mask = (1 - torch.eye(N).unsqueeze(0).unsqueeze(-1)).to(x_enc.device) # 1, N, N, 1
            lst = lst * mask # B, N, N, pn
        lst = lst.transpose(2,3)
        # lst = self.enc_emb(lst.transpose(2,3)) # B, N, pn, d_model
        
        # fi = torch.concat([lst, x_enc], dim=-1) # # B, N, pn, d_model*2
        # fi = self.forward_linear(fi) # d_model
        ...
        for i, block in enumerate(self.mid):
            # if i == 0:
            #     prev = self.norm(init+fi[:, :, i, :])
            # else:
            #     prev = self.norm(prev+fi[:, :, i, :])
            if i == 0:
                prev = block(x_enc[:,:, i, :] , init, lst[:, :, i, :]) # B*N d_model
            else:
                prev = block(x_enc[:,:, i, :], prev, lst[:, :, i, :]) # B*N d_model
            prevs.append(prev)
