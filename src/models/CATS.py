...
class Model_backbone(nn.Module):
    def __init__(self, c_in:int, seq_len:int, pred_len:int, patch_len:int=24, stride:int=24, n_layers:int=3, d_model=128, n_heads=16, d_ff:int=256, 
                 ...
                 QAM_end:float =0.5, padding_patch = None, shuffle=False, **kwargs):
        ...
        if self.shuffle:
            self.permuted_indices = torch.randperm(12)
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        if self.shuffle:
            # permuted_indices = torch.randperm(z.size(2))  # 生成打乱的索引
            z = z[: , :, torch.randperm(12), :]


        # model
        z = self.backbone(z)                                                                # z: [bs x nvars x pred_patch_num x d_model]
        z = self.proj(z)[:, :, -self.pred_len:]                                                                    # z: [bs x nvars x pred_len] 
...
class CATS(nn.Module):
    def __init__(self, dec_in, seq_len, pred_len, d_layers, n_heads, d_model, d_ff, dropout, query_independence, patch_len, stride,padding_patch, store_attn, QAM_start, QAM_end, shuffle=False, **kwargs):
        ...
        self.model = Model_backbone(c_in=c_in, seq_len = seq_len, pred_len=self.pred_len, patch_len=patch_len, stride=stride, n_layers=n_layers, 
                                    ...
                                    store_attn=store_attn, padding_patch = padding_patch, QAM_start=QAM_start, QAM_end=QAM_end, shuffle=self.shuffle, **kwargs)
