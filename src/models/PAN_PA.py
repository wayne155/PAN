...
class PatchAmplifier(nn.Module):
    def __init__(self, enc_in, patch_emb, in_size, device, patch_info=True, cd_info=True, out_size=512):
        ...
        l_in_size = ((patch_emb if patch_info else 0)  + (enc_in if cd_info else 0))
    ...
    def forward(self, patch, prev, ceb):
        ...
        if self.cd_info and self.patch_info:
            inp = torch.concat([patch, lst], dim=2) # B, N, N+I
        elif self.cd_info:
            inp = torch.concat([lst], dim=2) # B, N, N+I
        elif self.patch_info:
            inp = torch.concat([patch], dim=2) # B, N, N+I
        else:
            inp = torch.concat([prev], dim=2) # B, N, N+I
            
        out = self.norm(self.linear(inp))
        return out
...
class PAN(nn.Module):
    def __init__(self, seq_len,pred_len,enc_in, device, shared=True, sh_ch=False, sh_pa=False, raw_info=True, d_c=7, d_patch=128, flatten=False, cd_info=True, patch_info=True, revin=False, hidden_size=128,start_d_model=128,end_d_model=512 ,dropout=0.0, patch_len=96, stride=48, task_name="long_term_forecast"):
        """
        ...
        if self.sh_pa:
            self.sh_pa_per = torch.randperm(self.patch_num).to(device)
        if self.sh_ch:
            self.sh_ch_per = torch.randperm(seq_len+stride).to(device)
        ...
        if self.raw_info:
            ...
            if self.shared:
                self.mid = PatchAmplifier(enc_in=d_c, patch_emb=d_patch, patch_info=patch_info, cd_info=self.cd_info, device=self.device, in_size=end_d_model, out_size=end_d_model)

        else:
            self.value_embedding = nn.Linear(patch_len, d_patch, bias=False)
            self.channel_embedding = nn.Linear(enc_in, d_c, bias=False)
            print("not raw_info..")
            if self.shared:
                self.mid = PatchAmplifier(enc_in=d_c, patch_emb=d_patch, patch_info=patch_info, cd_info=self.cd_info, device=self.device, in_size=end_d_model, out_size=end_d_model)
            else:
                self.mid = nn.ModuleList(
                                [PatchAmplifier(enc_in=start_d_model, patch_emb=start_d_model, patch_info=patch_info, cd_info=self.cd_info, device=self.device, in_size=self.d_models[i], out_size=self.d_models[i+1]) for i, _ in enumerate(self.t_index)]
                            )
        print(f"{self.d_models}")
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if self.raw_info:
            ...
            if self.sh_pa:
                # permuted_indices = torch.randperm(x_enc.size(2))  # 生成打乱的索引
                x_enc = x_enc[:, : , self.sh_pa_per, :]
            ...
            x_enc = x_enc.permute(0, 2, 1) # B N T
            x_enc = self.padding_patch_layer(x_enc)
            channel_emb = self.channel_embedding(x_enc.transpose(1,2)) # B, T+pad, d_c
            x_enc = x_enc.unfold(dimension=-1, size=self.patch_len, step=self.stride)
            x_enc = torch.reshape(x_enc, (B, N, x_enc.shape[2], x_enc.shape[3]))
            # self.sh_pa_per = self.sh_pa_per.to(x_enc.device)
            # self.sh_ch_per = self.sh_ch_per.to(x_enc.device)

            if self.sh_pa:
                # permuted_indices = torch.randperm(x_enc.size(2))  # 生成打乱的索引
                x_enc = x_enc[:, : , self.sh_pa_per, :]
            if self.sh_ch:
                # permuted_indices = torch.randperm(channel_emb.size(1))  # 生成打乱的索引
                channel_emb = channel_emb[:, self.sh_ch_per, :]
            ...
            if self.shared:
                ...
                for i, block in enumerate(self.patch_num):
                    if i == 0:
                        prev = block(x_enc[:,:, i, :] , init, channel_emb[:, self.t_index[i], :]) # B N d_model
                    else:
                        prev = block(x_enc[:,:, i, :], prev, channel_emb[:, self.t_index[i], :]) # B N d_model
