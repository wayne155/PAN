...
class CiDBlock(nn.Module):
    def __init__(self,enc_in, patch_emb, in_size, mask=True, out_size=512, ti=95):
        super().__init__()
...
class CiDP(nn.Module):
    def __init__(self, seq_len,pred_len,enc_in, flatten=False, mask=True, hidden_size=128,start_d_model=256,end_d_model=256 ,dropout=0.0, patch_len=24, stride=16, task_name="long_term_forecast"):
        """
        ...
        self.mid = nn.ModuleList(
            [CiDBlock(enc_in=enc_in, patch_emb=patch_len, in_size=self.d_models[i], mask=mask, out_size=self.d_models[i+1], ti=ti) for i, ti in enumerate(self.t_index)]
        )
        if self.flatten:
            self.output = nn.Linear(sum(self.d_models), pred_len)
        else:
